...
class IndividualFinalizationService:
    """
    Handles INDIVIDUAL manager final decision.

    Flow:
    1. Validate manager payload
    2. Fetch model output from individual_manager_review_data
    3. Compute override flag
    4. Store final decision in shared msme_final_decision
    5. Update individual_manager_review_data.status -> reviewed
    6. Publish event for customer PDF generator
    """

    CUSTOMER_PDF_TOPIC = "customer_pdf"
    ...
    def validate_manager_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if decision == "APPROVED":
            ...
            # tenure optional; if not supplied, fallback to model/requested tenure
        else:
            principal = None
            interest_rate = None
            tenure = None
    ...
    def build_final_payload(
        ...
        model_tenure = review_row.get("loan_tenure_months")
        ...
        payload = {
            ...
            # review row info useful for downstream customer PDF
            "full_name": review_row.get("full_name"),
            "email": review_row.get("email"),
            "phone": review_row.get("phone"),
            "pan_number": review_row.get("pan_number"),
            "employment_status": review_row.get("employment_status"),
            "loan_ask": review_row.get("loan_ask"),
            "loan_tenure_months": review_row.get("loan_tenure_months"),
        ...
        try:
            ...
            cursor.execute(
                ...
                (
                    ...
                    final_payload["application_type"],
            ...
            cursor.execute(
                """
                UPDATE individual_manager_review_data
                SET status = 'reviewed',
                    updated_at = CURRENT_TIMESTAMP
                WHERE application_id = %s
                """,
    ...
    def publish_customer_pdf_request(self, final_payload: Dict[str, Any]) -> None:
        ...
        event = {
            ...
            "full_name": final_payload.get("full_name"),
            "email": final_payload.get("email"),
            "phone": final_payload.get("phone"),
            "pan_number": final_payload.get("pan_number"),
            "employment_status": final_payload.get("employment_status"),
            "loan_ask": final_payload.get("loan_ask"),
            "loan_tenure_months": final_payload.get("loan_tenure_months"),
        ...
        future = self.producer.send(self.CUSTOMER_PDF_TOPIC, event)
        record_metadata = future.get(timeout=15)

        print(
            f"[✔] Sent to {self.CUSTOMER_PDF_TOPIC} | "
    ...
    def finalize(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "message": "Individual final decision stored and customer PDF generation triggered.",
        }
...
# Singleton
# ----------------------------------------------------------------------
finalization_service = IndividualFinalizationService()